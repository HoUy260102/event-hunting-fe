  ...
  useEffect(() => {
    const fetchVoucher = async () => {
      try {
        ...
        // Determine if fields should be locked (ACTIVE status and startTime has passed)
        const isStarted = new Date(data.startTime) <= new Date();
        const activeAndStarted = data.status === "ACTIVE" && isStarted;
        setIsFieldsLocked(activeAndStarted);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setModal({
        isOpen: true,
        title: "Lỗi",
        message: serverError?.message || error.message || "Cập nhật voucher thất bại.",
        type: "error",
      });
    }
  };
      ...
      <main className="flex-1 p-6 lg:p-10 max-w-[1000px] mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white dark:bg-[#1a2c15] p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col gap-1 mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
            Chỉnh sửa thông tin chi tiết, phạm vi áp dụng và cấu hình mức giảm của khuyến mãi.
          </p>
        </div>
        ...
        >
          {/* Lock warning badge */}
          {isFieldsLocked && (
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-2xl p-5 flex items-start gap-3 animate-in fade-in duration-300">
              <span className="material-symbols-outlined text-amber-500 text-2xl mt-0.5 select-none">
                lock
              </span>
              <div>
                <h4 className="text-sm font-bold text-amber-800 dark:text-amber-300">
                  Khuyến mãi đang chạy và hoạt động!
                </h4>
                <p className="text-xs text-amber-700 dark:text-amber-400 mt-1 leading-relaxed font-medium">
                  Để đảm bảo tính nhất quán của các đơn hàng đã đặt, các trường quan trọng (loại giảm, giá trị giảm, mức áp dụng tối thiểu, giảm tối đa và các loại vé sự kiện) đã được **khóa tự động**. Bạn vẫn có thể thoải mái cập nhật tên, mã code, thời gian diễn ra và số lượng áp dụng.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className={`w-full h-11 px-4 rounded-xl border outline-none text-sm placeholder:text-slate-400 dark:placeholder:text-[#a1aebf] hover:border-slate-300 dark:hover:border-[#36532f] transition-all duration-200 ${
                  isFieldsLocked
                    ? "bg-slate-100/85 dark:bg-[#142210]/85 cursor-not-allowed text-slate-400 dark:text-slate-500 border-slate-200 dark:border-[#2a4225]"
                    : "bg-slate-50/50 dark:bg-[#142210]/50 border-slate-200 dark:border-[#2a4225] focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-500/20 text-slate-800 dark:text-white"
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex bg-slate-100 dark:bg-[#142210] rounded-xl p-1 shadow-inner gap-1 w-fit">
                  <button
                    ...
                    onClick={() => !isFieldsLocked && setDiscountType("VALUE")}
                    className={`px-4 py-2 text-xs font-bold rounded-lg transition-all duration-200 ${
                      isFieldsLocked ? "cursor-not-allowed opacity-60" : "cursor-pointer"
                    } ${discountType === "VALUE"
                  ...
                  <button
                    type="button"
                    disabled={isFieldsLocked}
                    onClick={() => !isFieldsLocked && setDiscountType("PERCENT")}
                    className={`px-4 py-2 text-xs font-bold rounded-lg transition-all duration-200 ${
                      isFieldsLocked ? "cursor-not-allowed opacity-60" : "cursor-pointer"
              ...
              <div className="flex flex-col gap-4 bg-slate-50/30 dark:bg-[#142210]/20 p-5 rounded-2xl border border-slate-100 dark:border-[#2a4225] mt-2">
                <label className="flex items-center gap-3 text-sm font-bold text-slate-700 dark:text-slate-300 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    disabled={isFieldsLocked}
                    className="w-5 h-5 rounded border-gray-300 dark:border-[#2a4225] text-emerald-500 focus:ring-emerald-500 cursor-pointer bg-white dark:bg-[#142210] disabled:cursor-not-allowed disabled:opacity-60"
                    checked={hasMax}
                    onChange={(e) => {
                    ...
                    <input
                      ...
                      className={`w-full h-11 px-4 rounded-xl border outline-none text-sm placeholder:text-slate-400 dark:placeholder:text-[#a1aebf] hover:border-slate-300 dark:hover:border-[#36532f] transition-all duration-200 ${
                        isFieldsLocked
                          ? "bg-slate-100/85 dark:bg-[#142210]/85 cursor-not-allowed text-slate-400 dark:text-slate-500 border-slate-200 dark:border-[#2a4225]"
                          : "bg-slate-50/50 dark:bg-[#142210]/50 border-slate-200 dark:border-[#2a4225] focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-500/20 text-slate-800 dark:text-white"
                      } ${errors.maxDiscountValue ? "border-red-500 focus:border-red-500 focus:ring-red-100" : ""}`}
                      placeholder="Ví dụ: 50000"
                      type="number"
                    />
          ...
          {/* SECTION 2: Điều kiện Vé (ORGANIZER) */}
                      ...
                      {tickets.map((ticket) => (
                        ...
                        >
                          <label className="flex items-center gap-4 p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5 transition-colors select-none">
                            <input
                              disabled={isFieldsLocked}
                              className="w-4.5 h-4.5 rounded border-gray-300 dark:border-[#2a4225] text-emerald-500 focus:ring-emerald-500 cursor-pointer bg-white dark:bg-[#142210] disabled:cursor-not-allowed disabled:opacity-60"
                              type="checkbox"
