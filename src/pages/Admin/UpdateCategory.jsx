...
function UpdateCategory() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const updatedCategory = response?.data; // Vì response là ApiResponse, data thực tế nằm ở .data!
      reset({
      ...
      console.error("Cập nhật chủ đề thất bại:", error);
      ...
      if (error && error.details) {
        ...
        setModal({
          isOpen: true,
          title: "Cập nhật chủ đề",
          message: error?.message || "Cập nhật chủ đề thất bại!",
          type: "error",
        });
      }
    } finally {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const categoryData = response.data; // Vì response là ApiResponse, data thực tế nằm ở .data!
      ...
      <main className="flex-1 p-6 lg:p-10 max-w-[1000px] mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white dark:bg-[#1a2c15] p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col gap-1 mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Cập nhật thông tin chi tiết và tên của chủ đề trong hệ thống.
          </p>
        </div>
